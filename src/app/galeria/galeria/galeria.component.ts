...
export class GaleriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.categoriaService.obterTodas().subscribe((categoriasResposta) => {
      this.categoriasFiltro = categoriasResposta;
      console.log('Categorias para filtro', this.categoriasFiltro);
    });

    this.lugarService.obterTodas().subscribe((lugaresResposta) => {
  ...
  getTotalEstrelas(lugar: Lugar): string {
    return (
      ...
      '&#9734'.repeat(5 - Number(lugar.avaliacao) || 5)
    );
  }
